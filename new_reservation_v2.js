...
'</div>'+
'<tab-content title="諮詢檢測" :before-change="checkStep1" :after-change="scroll_to_top">' +
...
'<div :class="\'mx-auto ps-4 py-4 \'" v-if="target_svc[\'Q4\'].length>0">' +
'<div colspan="1" role="group" class="row input-group mb-3 align-items-center">'+
'<template v-for="(row_style, style, style_i) in style_dict">' +
...
'<span>已依造您髮況所需，篩選五組適合的療程，請選擇其中一種</span>' +
'</div>' +
'</div>' +
'<div :class="chk_style(style)+\' zoss-svc-h p-0 row input-group align-items-center card\'" role="group">' +
'<div class="zoss_ultra card-header fs-6">' +
...
Vue.component('booking', {
    ...
    props: ['name', 'sex', 'tel', 'email', 'title', 'plan_dict', 'shop_list', 'activity', 'services', 'designation', 'suites'],
    data: function() {
        return {
            errorMsg:"",
            suite: "",
            ...
            target_products: [],
            style_dict: {"zoss_ultra": "row-cols-2", "zoss_lite": "row-cols-2"},
            shop_designers: {},
        }
    },
    filters: {
